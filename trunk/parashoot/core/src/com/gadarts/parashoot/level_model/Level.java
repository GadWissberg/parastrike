...
public class Level {
    ...
    private void removeEnemiesBySkill() {
        for (int i = 0; i < enemyAppearances.size; i++) {
            ...
            if (enemyAppearance.getMinimumSkill().compareTo(Main.getPlayerStatsHandler().getSelectedLevelSkill()) > 0) {
    ...
    public void start(WarScreenElements elements, Factories factories, HUD hud) {
        ...
        Main.getSoundPlayer().playMusic(music);
        levelTimer.start();
    ...
    private void advanceLevelsStates() {
        if (Main.getPlayerStatsHandler().getSelectedScene() == null) {
            return;
        }
        PlayerStatsHandler playerStatsHandler = Main.getPlayerStatsHandler();
    ...
    private void reportLevelBegin() {
        ...
        attributes.put(Rules.System.Analytics.Attributes.InGame.LEVEL_NUMBER, String.valueOf(Main.getPlayerStatsHandler().getSelectedLevel()));
        attributes.put(Rules.System.Analytics.Attributes.InGame.LEVEL_SKILL, Main.getPlayerStatsHandler().getSelectedLevelSkill().name());
        Main.getInstance().actionResolver.analyticsEventReport(Rules.System.Analytics.Events.LEVEL_BEGIN, attributes);
    }
    ...
    private void reportLevelEnd(States state) {
        ...
        Main.getInstance().actionResolver.analyticsEventReport(Rules.System.Analytics.Events.LEVEL_END, attributes);
    }
    ...
    private final Timer.Task TASK_SHOW_STATISTICS = new Timer.Task() {
        ...
        public void run() {
            ...
            Main.getGGS().submitScore(Main.getPlayerStatsHandler().getScore());
        }
    };
