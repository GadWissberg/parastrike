...
public class HeatTurret extends com.gadarts.parashoot.player.SideKick {
    ...
    public HeatTurret() {
        atlas = Main.getAssetsManager().get(Assets.GFX.Sheets.InGame.Player.SideKicks.HEAT_TURRET_DATA_FILE);
    }
    ...
    public boolean setTarget(Enemy target) {
        if (targets.size < Rules.Player.SideKicks.HeatTurret.MAX_TARGETS && !targets.contains((GroundUnit) target, true)) {
            if (targets.size == 0) {
                myBeamSoundId = Main.getSoundPlayer().playSound(SFX.Player.SideKicks.HEAT_BEAM, true);
            }
            ...
            Main.getSoundPlayer().playSound(SFX.Player.SideKicks.HEAT_BEAM_ON, false);
            return true;
        }
        return false;
    }
    ...
    private void stopBeamSound() {
        if (myBeamSoundId != -1) {
            Main.getSoundPlayer().stopSound(SFX.Player.SideKicks.HEAT_BEAM, myBeamSoundId);
            myBeamSoundId = 0;
        }
    }
