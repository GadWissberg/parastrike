...
public class Turret extends GameObject {
    ...
    public Turret(float x, float y, BulletType selectedWeapon, int generator, WarScreen warScreen) {
        super((TextureAtlas) Main.getAssetsManager().get(Assets.GFX.Sheets.InGame.Player.Bunker.Turret.DATA_FILE), null, x, y, warScreen.getElements());
    ...
    private void fire() {
        ...
        Main.getSoundPlayer().playSound(currentWeaponSound);
    }
    ...
    private void heatUp() {
        if (currentHeatUse != 0) {
            if (heatPoints + currentHeatUse < OVER_HEAT) {
                ...
                overHeat = true;
                Main.getMentorsManager().readyMentorIfDidntRun(UPGRADE_GENERATOR);
                warScreen.getHud().engageSmoke();
                warScreen.getElements().getMessageDisplay().add(Assets.Strings.InGameMessages.OVER_HEAT);
                Main.getSoundPlayer().playSound(SFX.Weapons.OVER_HEAT);
