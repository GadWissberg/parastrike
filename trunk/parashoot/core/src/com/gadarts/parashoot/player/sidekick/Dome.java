...
public class Dome extends com.gadarts.parashoot.player.SideKick {
    ...
    public Dome() {
        atlas = Main.getAssetsManager().get(Assets.GFX.Sheets.InGame.Player.SideKicks.DOME_DATA_FILE);
    }
    ...
    public void shoot() {
        if (isGunLoaded() && hasTarget()) {
            setGunState(false);
            Main.getSoundPlayer().playSound(SFX.Player.SideKicks.DOME_LOADING_PLASMA);
    ...
    private final Timer.Task CREATE_BULLET = new Timer.Task() {
        ...
        public void run() {
            if (target != null && !dead) {
                ...
                Main.getSoundPlayer().playSound(SFX.Weapons.HOMING_LASER);
