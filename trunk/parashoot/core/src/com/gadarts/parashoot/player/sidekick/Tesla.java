...
public class Tesla extends com.gadarts.parashoot.player.SideKick {
    ...
    public Tesla() {
        atlas = Main.getAssetsManager().get(Assets.GFX.Sheets.InGame.Player.SideKicks.TESLA_DATA_FILE);
    }
    ...
    public void shoot() {
        if (isGunLoaded() && !isDead()) {
            ...
            Main.getSoundPlayer().playSound(SFX.Player.SideKicks.TESLA_LOAD);
            setGunState(false);
    ...
    private final Timer.Task TASK_ELECTRIFY = new Timer.Task() {
        ...
        public void run() {
            ...
            Main.getSoundPlayer().playSound(SFX.Player.SideKicks.TESLA_SHOOT);
