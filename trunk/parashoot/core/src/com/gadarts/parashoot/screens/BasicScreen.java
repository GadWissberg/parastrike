...
public abstract class BasicScreen implements Screen {
    ...
    public BasicScreen() {
        Skin skin = new Skin();
        skin.addRegions(Main.getAssetsManager().get(Assets.GFX.Sheets.General.TOP_LOADING_SCREEN_DATA_FILE, TextureAtlas.class));
        skin.addRegions(Main.getAssetsManager().get(Assets.GFX.Sheets.General.BOTTOM_LOADING_SCREEN_DATA_FILE, TextureAtlas.class));
        loadingDoors = new LoadingsDoors(skin);
        armyFont = Main.getAssetsManager().get(Rules.System.FontsParameters.ArmyFontNames.HUGE, BitmapFont.class);
        digitalFont = Main.getAssetsManager().get(Rules.System.FontsParameters.DigitalFontNames.HUGE, BitmapFont.class);
        regularFont = Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.HUGE, BitmapFont.class);
        loadingDoors.forceLoadingDoorsState(Main.getAssetsManager().isLoadingInProcess());
        Main.getAssetsManager().setLoadingProcess(false);
    }
    ...
    protected class LoadingDoor extends Image {
        ...
        public void draw(Batch batch, float parentAlpha) {
            ...
            if (loadingRegion != null) {
                ...
                if (getY() == INSIDE_Y || !assetsManager.isLoadingInProcess()) {
                    ...
                    drawText(batch, Main.getAssetsManager().get(Rules.System.FontsParameters.ArmyFontNames.MEDIUM, BitmapFont.class), Rules.System.Resolution.WIDTH_TARGET_RESOLUTION / 2, getY() + getHeight(), Integer.toString(percent) + "...", true, Color.RED);
                }
            }
        }
        ...
        public void act(float delta) {
            ...
            if (onCompletionTask != null && loadingTask == null) {
                if (Main.getAssetsManager().update() && getY() == INSIDE_Y) {
                    if (loadingRegion != null) {
                        Main.getSoundPlayer().playSound(SFX.HUD.ENGINE_MOVE);
                    }
        ...
        private void handleMovement() {
            if (Main.getAssetsManager().isLoadingInProcess()) {
                if (!BOTTOM) {
                    if (getY() >= OUTSIDE_Y) {
        ...
        public void activate(Timer.Task loadingTask, Timer.Task onCompletionTask) {
            ...
            setVisible(true);
            Main.getAssetsManager().setLoadingProcess(true);
            if (loadingTask != null) {
        ...
        public void forceState(boolean state) {
            Main.getAssetsManager().setLoadingProcess(state);
