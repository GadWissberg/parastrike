...
public class LevelSelectionScreen extends MenuScreenImproved {
    ...
    public LevelSelectionScreen() {
        ...
        Scene selectedScene = Main.getPlayerStatsHandler().getSelectedScene();
        AssetManagerWrapper assetsManager = Main.getAssetsManager();
        NUM_OF_LEVELS_IN_SCENE = assetsManager.calculateNumberOfLevelsInScene(selectedScene);
        setBackScreen(Main.MenuType.LOBBY);
        MonitorImproved monitor = createMonitor(createMainMonitorContent(), Rules.Menu.SceneSelection.Monitor.MONITOR_WIDTH, Rules.Menu.SceneSelection.Monitor.MONITOR_HEIGHT, Rules.System.Resolution.HEIGHT_TARGET_RESOLUTION / 2 - Rules.Menu.SceneSelection.Monitor.MONITOR_HEIGHT / 2, true, true, false, Assets.Strings.Menu.LevelSelection.HEADER);
    ...
    private void createSceneCompletedMessage(Table table) {
        Label message = new Label(String.format(Assets.Strings.Menu.LevelSelection.SceneCompletedMonitor.TEXT, Main.getPlayerStatsHandler().getSelectedScene().getDisplayName(), Rules.Menu.LevelSelection.SceneCompletedMonitor.REWARD), new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.MEDIUM, BitmapFont.class), Color.GOLD));
    ...
    private WidgetGroup createMainMonitorContent() {
        Table table = new Table();
        table.add(new Label(Main.getPlayerStatsHandler().getSelectedScene().getDisplayName(), new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.ArmyFontNames.MEDIUM, BitmapFont.class), Color.GOLD))).colspan(NUM_OF_LEVELS_IN_SCENE).row();
    ...
    private void initializeAndCreateLevelsButtons(Table table) {
        ...
        Label.LabelStyle style = new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.ArmyFontNames.HUGE, BitmapFont.class), Color.GOLD);
    ...
    private ButtonClickImproved defineLevelButtonClick(final int levelIndex, final LevelState levelState) {
        return new ButtonClickImproved() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                final PlayerStats oldStats = new PlayerStats(Main.getPlayerStatsHandler().getPlayerStats());
                initializeLevel();
                reduceAmmo();
                goToBattle(oldStats);
            }
            ...
            private Timer.Task defineLoadingFinishedCallBack(final PlayerStats oldStats) {
                return new Timer.Task() {
                    @Override
                    public void run() {
                        Main.getInstance().goToBattle(oldStats);
                    }
                };
            }
            ...
            private Timer.Task defineLoadingCallBack() {
                return new Timer.Task() {
                    @Override
                    public void run() {
                        Main.getSoundPlayer().stopMusic();
                        Main.getAssetsManager().loadDataBattle(Main.getPlayerStatsHandler().getSelectedScene().ordinal() + 1, levelIndex);
                    }
                };
            }
