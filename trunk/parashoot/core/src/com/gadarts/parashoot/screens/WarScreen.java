...
public class WarScreen extends BasicScreen {
    ...
    public void hide() {
        ...
        clearPools();
        Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.MEDIUM, BitmapFont.class).getData().setScale(1);
    }
    ...
    private void returnToMenu() {
        ...
        closeDoorsAndGoToScreen(new Timer.Task() {
            @Override
            public void run() {
                Main.getAssetsManager().loadDataMenus();
            }
        }, TASK_RETURN_TO_MENU);
    }
    ...
    private static void handleGameOverSound() {
        SoundPlayer soundPlayer = Main.getSoundPlayer();
    ...
    private Timer.Task TASK_RETURN_TO_MENU = new Timer.Task() {
        ...
        private void decideWhereToGo() {
            if (sceneUnlockIndex > 0) {
                returnAndShowSomething(SceneSelectionScreen.EVENT_SCENE_UNLOCKED, sceneUnlockIndex, Main.MenuType.SCENE_SELECTION);
            } else if (sceneCompleted != null) {
                returnAndShowSomething(LevelSelectionScreen.EVENT_SCENE_COMPLETED, 1, Main.MenuType.LEVEL_SELECTION);
            } else Main.getInstance().goToMenuScreen(Main.MenuType.LOBBY);
        }

        private void returnAndShowSomething(String event, int data, Main.MenuType screenDestination) {
            HashMap<String, Integer> info = new HashMap<String, Integer>();
            info.put(event, data);
            Main.getInstance().goToMenuScreen(screenDestination, info);
        }
