...
public class OptionsScreen extends MenuScreenImproved {
    ...
    public OptionsScreen() {
        ...
        createMonitor(createMainContent(), Rules.Menu.Options.Monitor.MONITOR_WIDTH, Rules.Menu.Options.Monitor.MONITOR_HEIGHT, Rules.Menu.Help.Monitor.MONITOR_Y, true, true, false, Assets.Strings.Menu.Options.HEADER_OPTIONS);
    }

    private ScrollPane createMainContent() {
        Label.LabelStyle style = new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.SMALL, BitmapFont.class), Color.GOLD);
    ...
    private void createGeneralOptions(Table optionsTable, Label.LabelStyle style) {
        createHeader(new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.MEDIUM, BitmapFont.class), Color.GOLD), optionsTable, Assets.Strings.Menu.Options.GENERAL);
    ...
    private void createInGameOptions(Table optionsTable, Label.LabelStyle style) {
        createHeader(new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.MEDIUM, BitmapFont.class), Color.GOLD), optionsTable, Assets.Strings.Menu.Options.IN_GAME);
    ...
    private void createOptionForLoading(Label.LabelStyle style, Table optionsTable) {
        ...
        loadButton.setDisabled(!Main.getGGS().isSignedIn());
    ...
    private void createOptionForGoogleGames(Label.LabelStyle style, Table optionsTable) {
        ...
        Preferences preferences = Gdx.app.getPreferences(Assets.Configs.Preferences.Settings.PREF_SETTINGS);
        boolean checked = Main.getGGS().isSignedIn();
        ...
        return new ButtonClickImproved() {
            ...
            private ButtonClickImproved createDeleteConfirmationClick(final String name) {
                return new ButtonClickImproved() {
                    ...
                    public void clicked(InputEvent event, float x, float y) {
                        super.clicked(event, x, y);
                        Main.getPlayerStatsHandler().reset();
                        getMonitors().get(name).setActivation(false);
                    }
                };
            }
    ...
    private void createConfirmationLabel(Table table, String restartConfirmation) {
        Label areYouSure = new Label(restartConfirmation, new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.SMALL, BitmapFont.class), Color.GOLD));
