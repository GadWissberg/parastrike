...
public class SceneSelectionScreen extends MenuScreenImproved {
    ...
    public SceneSelectionScreen() {
        ...
        setNextScreen(Main.MenuType.LOBBY);
        setBackScreen(Main.MenuType.WELCOME);
        createMainMonitor();
    ...
    public void show() {
        ...
        for (selectedSceneIndex = scenes.length - 1; selectedSceneIndex >= 0; selectedSceneIndex--)
            if (Main.getPlayerStatsHandler().isSceneEnabled(scenes[selectedSceneIndex])) break;
        scrollToSpecificScene(selectedSceneIndex);
    }

    private void createMainMonitor() {
        scrollPane = createMainMonitorContent();
    ...
    private ScrollPane createMainMonitorContent() {
        Table table = createMainMonitorTable();
        ScrollPane scrollPane = new ScrollPane(table, getScrollPaneStyle());
        scrollPane.setFadeScrollBars(false);
        return scrollPane;
    }

    private Table createMainMonitorTable() {
        ...
        createMainMonitorTableContent(table);
        table.top();
        return table;
    }

    private void createMainMonitorTableContent(Table table) {
        Skin skin = getSkin();
    ...
    private void createRequirementsTables(Table table) {
        Label.LabelStyle style = new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.ArmyFontNames.MEDIUM, BitmapFont.class), Color.GOLD);
    ...
    private void createRequirementsTable(Table table, Label.LabelStyle style, Scene scene) {
        ...
        if (Main.getPlayerStatsHandler().isSceneEnabled(scene))
    ...
    private void createScenesButtons(Table table, Drawable lockDrawable) {
        Label.LabelStyle labelStyle = new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.ArmyFontNames.BIG, BitmapFont.class), Color.GOLD);
        for (final Scene scene : Scene.values()) {
    ...
    private void createSceneButton(Table table, Drawable lockDrawable, Label.LabelStyle labelStyle, Scene scene) {
        ImageButton sceneButton = new ImageButton(new TextureRegionDrawable(new TextureRegion(Main.getAssetsManager().get(scene.getThumb(), Texture.class))));
    ...
    private void setCorrectSceneButtonClickListener(Drawable lockDrawable, Scene scene, Stack stack) {
        boolean sceneEnabled = Main.getPlayerStatsHandler().isSceneEnabled(scene);
    ...
    private ButtonClickImproved defineSceneButtonClick(final Scene scene, final boolean sceneEnabled) {
        return new ButtonClickImproved() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                if (sceneEnabled) {
                    Main.getPlayerStatsHandler().setSelectedScene(scene);
    ...
    private Table createLockedSceneTable(Scene scene) {
        Table table = new Table();
        Label.LabelStyle style = new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.MEDIUM, BitmapFont.class), Color.GOLD);
    ...
    private void createLockedSceneMonitorRequirementsLabels(Scene scene, Table table, Label.LabelStyle style) {
        Label.LabelStyle smallerStyle = new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.SMALL, BitmapFont.class), Color.GOLD);
        createLockedSceneMonitorRequirementLabel(scene.getStarsTarget(), table, style, Main.getPlayerStatsHandler().getStars(), Assets.GFX.Sheets.ImagesNames.STAR_LEVEL_FILLED, Rules.Menu.SceneSelection.Monitor.LockedSceneMonitor.STARS_ICON_PADDING_RIGHT, smallerStyle);
        createLockedSceneMonitorRequirementLabel(scene.getCoinsTarget(), table, style, Main.getPlayerStatsHandler().getCoins(), Assets.GFX.Sheets.ImagesNames.SMALL_COIN_ICON, Rules.Menu.SceneSelection.Monitor.LockedSceneMonitor.COINS_ICON_PADDING_RIGHT, smallerStyle);
    }
