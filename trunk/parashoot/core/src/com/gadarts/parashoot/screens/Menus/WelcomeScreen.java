...
public class WelcomeScreen extends MenuScreenImproved {
    ...
    public WelcomeScreen() {
        ...
        createLogo();
        setNextScreen(Main.MenuType.SCENE_SELECTION);
        playMusic();
        monitor = createMonitor(createMainContent(), MainMonitor.MONITOR_WIDTH, MainMonitor.MONITOR_HEIGHT, MainMonitor.MONITOR_Y, true, true, false);
        ...
        createButtons();
        Label.LabelStyle style = new Label.LabelStyle(Main.getAssetsManager().get(MEDIUM, BitmapFont.class), Color.GOLD);
        try {
            Label label = new Label(Main.getInstance().actionResolver.getAppVersion(), style);
    ...
    private void createLogo() {
        Texture texture = Main.getAssetsManager().get(Assets.GFX.Images.Menus.LOGO);
    ...
    private Label createThanksLabel() {
        Label thanksText = new Label(PLAY_STORE_TEXT, new Label.LabelStyle(Main.getAssetsManager().get(MEDIUM, BitmapFont.class), Color.GOLD));
    ...
    private Table createMainContent() {
        ...
        createMainMonitorButtons(table);
        table.setDebug(GameSettings.SHOW_TABLES_LINES);
        return table;
    }

    private void createMainMonitorButtons(Table table) {
    ...
    private void createOptionsButton(Table table) {
        ...
        button.addListener(defineMainMonitorButton(Main.MenuType.OPTIONS));
        table.add(button);
        table.row();
    }
    ...
    private void createHelpButton(Table table) {
        ...
        button.addListener(defineMainMonitorButton(Main.MenuType.HELP));
        table.add(button);
    }
    ...
    private void createPlayButton(Table table) {
        ...
        playButton.addListener(defineMainMonitorButton(getNextScreen()));
        table.add(playButton).colspan(2).padBottom(MainMonitor.PLAY_PADDING_BOTTOM);
        table.row();
    }

    private ButtonClickImproved defineMainMonitorButton(final Main.MenuType screen) {
        return new ButtonClickImproved() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                super.clicked(event, x, y);
                Main.getInstance().goToMenuScreen(screen);
            }
        };
    }
    ...
    private void playMusic() {
        if (!Main.getSoundPlayer().isMusicPlaying()) {
            Main.getSoundPlayer().playMusic(SFX.Music.MENU);
        }
    }
    ...
    private class RatingStarsGroup extends Table {
        ...
        private void reportRating() {
            ...
            Main.getInstance().actionResolver.analyticsEventReport(Rules.System.Analytics.Events.USER_RATED, attributes);
        }
