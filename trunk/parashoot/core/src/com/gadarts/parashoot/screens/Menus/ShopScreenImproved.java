...
public class ShopScreenImproved extends MenuScreenImproved {
    ...
    public void show() {
        ...
        Main.getMentorsManager().readyMentorIfDidntRun(SCROLLABLE);
        scheduleMentor(SCROLLABLE, TASK_SHOW_SCROLLABLE_MENTOR);
    }

    private void initializeShopScreen() {
        setBackScreen(Main.MenuType.LOBBY);
        addFontsToSkin();
        createContents();
        setDefaultSelection();
    }
    ...
    private void addFontsToSkin() {
        Skin skin = getSkin();
        skin.add(Rules.System.FontsParameters.DigitalFontNames.SMALL, new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.DigitalFontNames.SMALL, BitmapFont.class), Color.GOLD));
        skin.add(Rules.System.FontsParameters.DigitalFontNames.TINY, new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.DigitalFontNames.TINY, BitmapFont.class), Color.GOLD));
        addArmyFontsToSkin();
        skin.add(Rules.System.FontsParameters.RegularFontNames.TINY, new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.TINY, BitmapFont.class), Color.GOLD));
        addRegularSmallFontsToSkin();
    }

    private void addArmyFontsToSkin() {
        Skin skin = getSkin();
        skin.add(Rules.System.FontsParameters.ArmyFontNames.MEDIUM, new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.ArmyFontNames.MEDIUM, BitmapFont.class), Color.GOLD));
        skin.add(Rules.System.FontsParameters.ArmyFontNames.HUGE, new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.ArmyFontNames.HUGE, BitmapFont.class), Color.GOLD));
    }

    private void addRegularSmallFontsToSkin() {
        Skin skin = getSkin();
        skin.add(Rules.System.FontsParameters.RegularFontNames.SMALL, new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.SMALL, BitmapFont.class), Color.GOLD));
        skin.add(Rules.System.FontsParameters.RegularFontNames.SMALL + Color.GREEN.toString(), new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.SMALL, BitmapFont.class), Color.GREEN));
        skin.add(Rules.System.FontsParameters.RegularFontNames.SMALL + Color.RED.toString(), new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.SMALL, BitmapFont.class), Color.RED));
        skin.add(Rules.System.FontsParameters.RegularFontNames.MEDIUM + Color.RED.toString(), new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.MEDIUM, BitmapFont.class), Color.RED));
    }
    ...
    private void createTopRightLabelAndCoinsButton(Table topRightTable) {
        Skin skin = getSkin();
        currentCoinsLabel = new Label(Integer.toString(Main.getPlayerStatsHandler().getCoins()), skin.get(Rules.System.FontsParameters.DigitalFontNames.SMALL, Label.LabelStyle.class));
    ...
    private void createMainContents(Table table) {
        mainContent = new MainContent(getSkin());
    ...
    public SideKickFactory.SideKickType getStrogestEnabledSideKick() {
        ...
        for (int i = sideKicks.length - 1; i >= 0; i--) {
            ...
            if (Main.getPlayerStatsHandler().getSideKickAttribute(sideKick, Rules.Player.UpgradeableAttribute.WeaponAttribute.ENABLED) == 1)
                break;
        }
        return sideKick;
    }
    ...
    public BulletType getStrogestEnabledCannon() {
        ...
        for (int i = cannons.length - 1; i >= 0; i--) {
            cannon = cannons[i];
            if (Main.getPlayerStatsHandler().getCannonAttribute(cannon, Rules.Player.UpgradeableAttribute.WeaponAttribute.ENABLED) == 1)
                break;
        }
        return cannon;
    }
    ...
    private class MainContent extends ScrollPane {
        ...
        private ButtonClickImproved definePurchaseButtonClick() {
            return new ButtonClickImproved() {
                ...
                public void clicked(InputEvent event, float x, float y) {
                    super.clicked(event, x, y);
                    if (Main.getPlayerStatsHandler().getCoins() >= selectedItem.getCost()) {
                        performPurchase();
                ...
                private void purchaseCannon() {
                    ...
                    if ((Main.getPlayerStatsHandler().getBunkerGeneratorLevel() / 10) + 1 >= minGeneratorReq) {
                        updateCannonStats();
                        purchaseSuccess(Rules.System.Analytics.Events.CANNON_PURCHASED);
                        Main.getMentorsManager().readyMentorIfDidntRun(CHANGE_CANNON);
                    } else
                ...
                private void purchaseSuccess(String event) {
                    refresh();
                    Main.getSoundPlayer().playSound(SFX.Menu.SYSTEM_CONFIRM, false, false);
                    PlayerStatsHandler.reportEvent(event);
                    Main.getGGS().setShouldSaveGame(true);
                    currentCoinsLabel.setText(Integer.toString(Main.getPlayerStatsHandler().getCoins()));
                ...
                private void updateCannonStats() {
                    ...
                    PlayerStatsHandler playerStatsHandler = Main.getPlayerStatsHandler();
                ...
                private void updateSideKickStats() {
                    ...
                    playerStatsHandler.setCoins(playerStatsHandler.getCoins() - selectedItem.getCost());
                    Main.getMentorsManager().readyMentorIfDidntRun(Names.CHANGE_SIDE_KICK);
                }
        ...
        private boolean isSelectedItemEnabled() {
            if (selectedCategory == Category.CANNONS) {
                return Main.getPlayerStatsHandler().isCannonEnabled(selectedItem);
            } else if (selectedCategory == Category.SIDE_KICKS) {
                return Main.getPlayerStatsHandler().isSideKickEnabled(selectedItem);
            }
        ...
        private void refreshMainContent(boolean enabled) {
            description.setText(selectedItem.getDescription());
            if (enabled) {
                refreshMainContentForEnabled(selectedItem.getAttributes());
            } else refreshMainContentForNotEnabled();
        ...
        private void refreshMainContentForEnabled(Rules.Player.UpgradeableAttribute[] attributes) {
        ...
        private void refreshMainContentForNotEnabled() {
            purchasePrice.setText(String.format(ShopImproved.Monitor.PRICE, selectedItem.getCost()));
            if (selectedCategory == Category.CANNONS) {
                refreshMainContentForNotEnabledCannon();
            } else if (selectedCategory == Category.ARMORY) {
                refreshMainContentForBombs();
            } else purchaseAdditionalLabel.setVisible(false);
        }

        private void refreshMainContentForBombs() {
            Rules.Player.ArmoryItem selectedBomb = (Rules.Player.ArmoryItem) ShopScreenImproved.this.selectedItem;
            initializePurchaseAdditionalLabel(true, String.format(ShopImproved.Monitor.YOU_HAVE, Main.getPlayerStatsHandler().getBombAmount(selectedBomb), Rules.Player.Bonus.MAX_BOMBS_TO_HAVE), Rules.System.FontsParameters.RegularFontNames.SMALL);
        }

        private void refreshMainContentForNotEnabledCannon() {
            int minGeneratorReq = ((BulletType) selectedItem).getMinGeneratorReq();
            initializePurchaseAdditionalLabel(minGeneratorReq > 0, String.format(ShopImproved.Monitor.UPGRADE_GEN, minGeneratorReq), Rules.System.FontsParameters.RegularFontNames.SMALL + ((minGeneratorReq > (Main.getPlayerStatsHandler().getBunkerGeneratorLevel() / 10) + 1) ? Color.RED.toString() : ""));
        }
        ...
        private class Upgrader extends WidgetGroup {
            ...
            private abstract class UpgradeButton extends ImageButton {
                ...
                protected void setCannonAttribute() {
                    upgrader.setVisible(true);
                    attributeValue = Main.getPlayerStatsHandler().getCannonAttribute((BulletType) selectedItem, (Rules.Player.UpgradeableAttribute.WeaponAttribute) attributeType);
                }
                ...
                protected void setSideKickAttribute() {
                    ...
                    attributeValue = Main.getPlayerStatsHandler().getSideKickAttribute(selectedItem, attributeType);
                }
                ...
                protected void setArmoryAttribute() {
                    ...
                    upgrader.setVisible(true);
                    if (selectedItem == Rules.Player.ArmoryItem.BUNKER) {
                        attributeValue = Main.getPlayerStatsHandler().getBunkerAttribute(attributeType);
                    } else
                        attributeValue = Main.getPlayerStatsHandler().getBombAmount(selectedItem);
                }
    ...
    private void showError(String message) {
        Main.getSoundPlayer().playSound(SFX.Menu.SYSTEM_ERROR, false, false);
