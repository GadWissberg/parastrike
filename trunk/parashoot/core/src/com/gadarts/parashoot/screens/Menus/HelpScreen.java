...
public class HelpScreen extends MenuScreenImproved {
    ...
    public HelpScreen() {
        ...
        monitor = createMonitor(createMainContent(), Rules.Menu.Help.Monitor.MONITOR_WIDTH, Rules.Menu.Help.Monitor.MONITOR_HEIGHT, Rules.Menu.Help.Monitor.MONITOR_Y, false, true, false, Assets.Strings.Menu.Help.HEADER_HOW_TO_PLAY);
    ...
    private Table createMainContent() {
        createLayouts();
        Table table = createMainContentTable();
    ...
    private Table createMainContentTable() {
    ...
    private void createAboutMe(Table table, Label.LabelStyle style) {
        table.add(new Label(Assets.Strings.Menu.Help.ABOUT_ME_HEADER, new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.MEDIUM, BitmapFont.class), Color.GOLD))).colspan(2).padTop(Rules.Menu.Help.Monitor.ABOUT_ME_HEADER_PADDING_TOP).left();
        table.row();
    ...
    private ButtonClickImproved defineLogoClick() {
        return new ButtonClickImproved() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                super.clicked(event, x, y);
                Main.getInstance().actionResolver.mail();
            }
        };
    }
