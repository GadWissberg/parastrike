...
import static com.gadarts.parashoot.utils.Rules.Menu.Lobby.MainMonitor.*;
import static com.gadarts.parashoot.utils.Rules.Menu.Lobby.MainMonitor.AttackButton.*;
import static com.gadarts.parashoot.utils.Rules.Menu.Lobby.MainMonitor.InfoBorders.*;
...
public class LobbyScreen extends MenuScreenImproved {
    ...
    private void checkDailyGift() {
        ...
        Main.getInstance().actionResolver.toast("SENT");
    ...
    private ButtonClickImproved defineGiftClick(final Gift gift) {
        return new ButtonClickImproved() {
            ...
            private WidgetGroup defineGiftMonitorContent(Gift gift) {
                ...
                BitmapFont font = Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.SMALL, BitmapFont.class);
                    ...
                    private Net.HttpResponseListener defineResponseListener() {
                        return new Net.HttpResponseListener() {
                            ...
                            private void fail() {
                                ...
                                Main.getSoundPlayer().playSound(SYSTEM_ERROR);
                            }
                            ...
                            private void gainCoins() {
                                ...
                                Main.getPlayerStatsHandler().commitStats();
                                Main.getSoundPlayer().playSound(SYSTEM_CONFIRM);
            ...
            private ButtonClickImproved defineFacebookButtonClick() {
                return new ButtonClickImproved() {
                    ...
                    public void clicked(InputEvent event, float x, float y) {
                        super.clicked(event, x, y);
                        Main.getInstance().getActionResolver().openUrl(Assets.Strings.FACEBOOK_PAGE);
                    }
                };
            }
    ...
    private final Timer.Task TASK_FLICKER_CANNON = new Timer.Task() {
        ...
        public void run() {
            ...
            if (bunkerScheme.cannonInfoBorder.isVisible()) {
                Main.getSoundPlayer().playSound(SYSTEM_WARNING, false, false);
            }
        }
    };
    ...
    private void correctCannonSelection(PlayerStatsHandler playerStatsHandler) {
        ...
        Main.getMentorsManager().readyMentorIfDidntRun(AMMUNITION_DEPLETED);
    }
    ...
    private void createMainMonitor() {
        monitor = createMonitor(createMainContent(), MONITOR_WIDTH, MONITOR_HEIGHT, MONITOR_Y, false, true, false, Lobby.HEADER);
        addCloudSavedImage();
        addAttackButton(monitor);
    }

    private void setScreens() {
        setNextScreen(Main.MenuType.LEVEL_SELECTION);
        setBackScreen(Main.MenuType.SCENE_SELECTION);
    }

    private void createRegularTextStyle() {
        BitmapFont font = Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.TINY, BitmapFont.class);
    ...
    private ButtonClickImproved defineAttackButtonClick() {
        return new ButtonClickImproved() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                super.clicked(event, x, y);
                Main.getInstance().goToMenuScreen(getNextScreen());
            }
        };
    }

    private WidgetGroup createMainContent() {
        Skin skin = getSkin();
    ...
    public void show() {
        super.show();
        cloudSaveGame();
        Main.getMentorsManager().readyMentorIfDidntRun(GO_TO_BATTLE);
        scheduleMentor(GO_TO_BATTLE, TASK_MENTOR_GO_TO_BATTLE);
    }

    private void cloudSaveGame() {
        GGSActionResolver ggs = Main.getGGS();
        Preferences preferences = Gdx.app.getPreferences(PREF_SETTINGS);
    ...
    protected void onBackPressed() {
        Main.getInstance().goToMenuScreen(getBackScreen());
    }
    ...
    private void goToShop(ShopScreenImproved.Category category, int selectedIndex) {
        ...
        Main.getInstance().goToMenuScreen(Main.MenuType.SHOP, additionalInfo);
    }
    ...
    private class BunkerScheme extends Image {
        ...
        private void addAmmoValue(Table layout) {
            PlayerStatsHandler playerStatsHandler = Main.getPlayerStatsHandler();
            ammoLabel = new Label(null, new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.RegularFontNames.SMALL, BitmapFont.class), Color.GOLD));
            ...
            ammoLabel.setPosition(Ammo.VALUE_LABEL_X, Ammo.VALUE_LABEL_Y);
        }
        ...
        private void addAmmoHeader(Table layout) {
            ...
            ammoHeader.setPosition(Ammo.HEADER_LABEL_X, Ammo.HEADER_LABEL_Y);
        }
        ...
        private void createSideKickInfoBorder() {
            ...
            sideKicksInfoBorder.putWeapon(Main.getPlayerStatsHandler().getSelectedSideKick());
        ...
        private void createBombsInfoBorder() {
            ...
            bombsInfoBorder.addAttribute(Shop.ArmoryNames.ShortArmoryNames.BIO, Assets.GFX.Sheets.ImagesNames.SMALL_BIO_ICON, String.valueOf(Main.getPlayerStatsHandler().getBombAmount(Rules.Player.ArmoryItem.BIO_HAZARD)));
            bombsInfoBorder.addAttribute(Shop.ArmoryNames.ShortArmoryNames.AIR, Assets.GFX.Sheets.ImagesNames.SMALL_BOMB_ICON, String.valueOf(Main.getPlayerStatsHandler().getBombAmount(Rules.Player.ArmoryItem.AIR_STRIKE)));
            bombsInfoBorder.addAttribute(Shop.ArmoryNames.ShortArmoryNames.ATOM, Assets.GFX.Sheets.ImagesNames.SMALL_ATOM_ICON, String.valueOf(Main.getPlayerStatsHandler().getBombAmount(Rules.Player.ArmoryItem.ATOM)));
        ...
        private void createCannonInfoBorder() {
            ...
            cannonInfoBorder.putWeapon(Main.getPlayerStatsHandler().getSelectedCannon());
    ...
    private class WeaponSelectionButton extends Button {
        ...
        private ClickListener defineClick(final WeaponType weaponType) {
            return new ClickListener() {
                ...
                public void clicked(InputEvent event, float x, float y) {
                    super.clicked(event, x, y);
                    if (weaponType instanceof BulletType && Main.getPlayerStatsHandler().getCannonAmmo((BulletType) weaponType) == 0) {
        ...
        private void sideKickClick(WeaponType weaponType) {
            Main.getPlayerStatsHandler().setSelectedSideKick(weaponType);
        ...
        private void cannonClick(WeaponType weaponType) {
            Main.getPlayerStatsHandler().setSelectedCannon(weaponType);
            cannonSelectionMonitor.setActivation(false);
            bunkerScheme.cannonInfoBorder.refresh(weaponType);
            bunkerScheme.setAmmoText(Main.getPlayerStatsHandler().getCannonAmmo((BulletType) weaponType));
        }
