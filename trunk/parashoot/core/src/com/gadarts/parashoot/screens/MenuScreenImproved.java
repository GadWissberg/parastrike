...
import static com.gadarts.parashoot.model.Main.MenuType.*;
...
public abstract class MenuScreenImproved extends BasicMenuScreen {
    ...
    private static final Timer.Task TASK_AUTO_PAUSE_SOUND_POOL = new Timer.Task() {
        @Override
        public void run() {
            Main.getInstance().getActionResolver().autoPauseSoundPool();
        }
    };

    private final Skin skin = new Skin(Main.getAssetsManager().get(Assets.GFX.Sheets.Menus.GENERAL_MENU_DATA_FILE, TextureAtlas.class));
    ...
    public MenuScreenImproved(boolean animatedBackground, String... imageSheets) {
        ...
        for (String imageSheet : imageSheets)
            skin.addRegions(Main.getAssetsManager().get(imageSheet, TextureAtlas.class));
        getSkin().add(MEDIUM, new Label.LabelStyle(Main.getAssetsManager().get(MEDIUM, BitmapFont.class), Color.GOLD));
    }
...
//            backgroundTexture = Main.getAssetsManager().get(Assets.GFX.Images.Menus.MAIN_THEME);
    ...
    private void setAnimatedBackground() {
        backgroundTexture = Main.getAssetsManager().get(Assets.GFX.Images.General.NIGHT);
        paratroopersBackground = Main.getAssetsManager().get(Assets.Configs.CONFIGS_FOLDER_NAME + "/" + Assets.Configs.PARTICLE_CONFIGS_FOLDER_NAME + "/" + Assets.Configs.ParticleEffects.MENU_PARATROOPERS + "." + Assets.Configs.PARTICLE_CONFIGS_DATA_EXTENSION, ParticleEffect.class);
    ...
    protected void createStarsDisplay(MonitorImproved monitor) {
        ...
        starsTable.add(new Label(Integer.toString(Main.getPlayerStatsHandler().getStars()), new Label.LabelStyle(Main.getAssetsManager().get(Rules.System.FontsParameters.ArmyFontNames.MEDIUM, BitmapFont.class), Color.GOLD))).padRight(Rules.Menu.SceneSelection.Monitor.StarsDisplay.STARS_VALUE_PADDING_RIGHT).padLeft(Rules.Menu.SceneSelection.Monitor.StarsDisplay.STARS_VALUE_PADDING_LEFT);
    ...
    private ButtonClickImproved defineStarsDisplayClick() {
        return new ButtonClickImproved() {
            ...
            private void createLabel(Table table) {
                Label label = new Label(Assets.Strings.Menu.SceneSelection.StarsInfoMonitor.TEXT, new Label.LabelStyle(Main.getAssetsManager().get(SMALL, BitmapFont.class), Color.GOLD));
    ...
    protected void createHoorayEffect(SFX.Menu sound) {
        hoorayEffect = Main.getAssetsManager().get(Assets.Configs.CONFIGS_FOLDER_NAME + "/" + Assets.Configs.PARTICLE_CONFIGS_FOLDER_NAME + "/" + Assets.Configs.ParticleEffects.SCENE_UNLOCKED_STARS + "." + Assets.Configs.PARTICLE_CONFIGS_DATA_EXTENSION, ParticleEffect.class);
        hoorayEffect.setPosition(0, Rules.System.Resolution.HEIGHT_TARGET_RESOLUTION);
        hoorayEffect.start();
        Main.getSoundPlayer().playSound(sound);
    }
    ...
    public class CoinsButton extends ImageButton {
        ...
        private ButtonClickImproved defineCoinsButtonClick() {
            return new ButtonClickImproved() {
                ...
                public void clicked(InputEvent event, float x, float y) {
                    ...
                    createCoinsMonitor();
                    Main.getGGS().queryProducts(names, CoinsButton.this);
                }
        ...
        public void createAndSetCoinsMonitorContent(HashMap<String, PurchaseItem> purchaseItems) {
            ...
            table.top();
            BitmapFont tinyFont = Main.getAssetsManager().get(TINY, BitmapFont.class);
            BitmapFont smallFont = Main.getAssetsManager().get(SMALL, BitmapFont.class);
        ...
        private Stack createBuyButton(Drawable buttonDrawable, Drawable buttonPressedDrawable,
            ...
            button.addListener(new ButtonClickImproved() {
                ...
                public void clicked(InputEvent event, float x, float y) {
                    super.clicked(event, x, y);
                    Main.getGGS().buy(purchaseItem.getId());
                }
            });
            return stack;
        }
        ...
        private Label.LabelStyle createStyle() {
            BitmapFont font = Main.getAssetsManager().get(SMALL, BitmapFont.class);
        ...
        public void purchaseSuccess(String id) {
            ...
            timer.scheduleTask(new Timer.Task() {
                @Override
                public void run() {
                    Main.MenuType navigation = getNextScreen() == LEVEL_SELECTION ? SHOP : LOBBY;
                    Main.getInstance().goToMenuScreen(navigation);
                }
