...
public class Chaingunner extends Paratrooper {
    ...
    public Chaingunner() {
        super();
        atlas = Main.getAssetsManager().get(Assets.GFX.Sheets.InGame.Enemies.GroundUnits.Paratroopers.CHAINGUNNER_DATA_FILE);
        ...
        public void run() {
            if (!dead && hasLanded() && target != null && !target.isDead()) {
                ...
                if (shootingSoundId == -1) {
                    shootingSoundId = Main.getSoundPlayer().playSound(SFX.Enemies.GroundUnits.Paratroopers.CHAINGUNNER_SHOOT, true);
                }
            }
        }
    };
    ...
    protected void onGib() {
        super.onGib();
        Main.getSoundPlayer().stopSound(SFX.Enemies.GroundUnits.Paratroopers.CHAINGUNNER_SHOOT, shootingSoundId);
    }
