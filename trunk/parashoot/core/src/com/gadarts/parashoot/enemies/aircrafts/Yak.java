...
public class Yak extends AirCraft {
    ...
    public Yak() {
        super();
        atlas = Main.getAssetsManager().get(Assets.GFX.Sheets.InGame.Enemies.AirCrafts.YAK_DATA_FILE);
    ...
    protected void init(float x, float y, float speed, float hp, Type type, WarScreenElements mechanics, PlayerHandler playerHandler, Factories factories) {
        ...
        myFlyBySound = Main.getSoundPlayer().playSound(SFX.Enemies.AirCrafts.PLANE_FLYBY);
        gunLoaded = true;
    }
    ...
    protected void onDeath() {
        Main.getSoundPlayer().stopSound(SFX.Enemies.AirCrafts.PLANE_FLYBY, myFlyBySound);
        if (type != Type.MEDIC) {
            super.onDeath();
        } else {
            onGib();
        ...
        if (gunLoaded && !dead && !getTarget().isDead()) {
            ...
            switch (type) {
                case REGULAR:
                    ...
                    bullet.setDamageValue(getDamageBySkill(0));
                    Main.getSoundPlayer().playSound(SFX.Player.SideKicks.AUTO_TURRET_SHOOT);
                    break;
                case SUPER:
                    ...
                    bullet.setDamageValue(getDamageBySkill(0));
                    bullet.setSpeed(Rules.Enemies.AirCrafts.Yak.MISSILE_SPEED);
                    Main.getSoundPlayer().playSound(SFX.Weapons.MISSILE_LAUNCH);
                    break;
            }
        }
    }
    ...
    public int getCoinsBySkill() {
        if (type != null) {
            switch (type) {
                case REGULAR:
                    if (Main.getPlayerStatsHandler().getSelectedLevelSkill().ordinal() == 0) {
                        return Rules.Enemies.AirCrafts.Yak.Easy.COINS;
                    } else if (Main.getPlayerStatsHandler().getSelectedLevelSkill().ordinal() == 1) {
