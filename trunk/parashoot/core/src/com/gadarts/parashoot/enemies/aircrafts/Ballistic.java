...
public class Ballistic extends AirCraft {
    ...
    protected void onOrderFulfilled() {
        ...
        myFallingSoundId = Main.getSoundPlayer().playSound(SFX.Enemies.AirCrafts.FALLING_BALLISTIC);
        stopMotorSound();
    }
    ...
    protected void onGib() {
        ...
        destroySmoke();
        onDestroy();
        Main.getSoundPlayer().stopSound(SFX.Enemies.AirCrafts.FALLING_BALLISTIC, myFallingSoundId);
    }
    ...
    public float getHealthBySkill() {
        int ordinal = Main.getPlayerStatsHandler().getSelectedLevelSkill().ordinal();
        if (ordinal == 0) {
