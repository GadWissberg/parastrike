...
public abstract class AirCraft extends Enemy {
    ...
    private void manageFallingWhenDead() {
        crashSoundId = Main.getSoundPlayer().playSound(crashSoundFileName);
        setRegion(Assets.GFX.Sheets.ImagesNames.FALLING);
        Main.getSoundPlayer().playSound(SFX.Misc.HIT);
        gravityStatus = true;
    ...
    protected void generateBonus() {
        ...
        if (!(bonus == BonusFactory.BonusType.SIDEKICK_UPGRADE && Main.getPlayerStatsHandler().getSelectedSideKick() == null) && !(bonus == BonusFactory.BonusType.AMMO && Main.getPlayerStatsHandler().getSelectedCannon() == BulletType.CANNON_BALL)) {
    ...
    protected void createExplosions() {
        ...
        Main.getSoundPlayer().playSound(SFX.Misc.BIG_EXPLOSION);
    ...
    public void onDestroy(boolean doDestroySequence) {
        ...
        Main.getSoundPlayer().stopSound(crashSoundFileName, crashSoundId);
        stopMotorSound();
