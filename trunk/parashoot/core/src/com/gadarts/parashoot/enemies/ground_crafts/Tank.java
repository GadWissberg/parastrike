...
public class Tank extends GroundCraft implements Targeting<PlayerCharacter> {
    ...
    public Tank() {
        atlas = Main.getAssetsManager().get(Assets.GFX.Sheets.InGame.Enemies.GroundUnits.GroundCrafts.TANK_DATA_FILE);
    ...
    private final Timer.Task TASK_SHOOT = new Timer.Task() {
        ...
        public void run() {
            if (!dead && target != null && !target.isDead()) {
                ...
                Main.getSoundPlayer().playSound(SFX.Enemies.GroundUnits.GroundCrafts.TANK_SHOOT);
            }
        }
    };
