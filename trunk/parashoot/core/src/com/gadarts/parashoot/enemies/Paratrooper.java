...
public abstract class Paratrooper extends GroundUnit {
    ...
    private void playRandomDyingSound() {
        ...
        Main.getSoundPlayer().playSound(selectedSound);
    }
    ...
    private void handleDieSprite() {
        if (lastDamagedBy == Rules.Enemies.GroundUnits.Paratroopers.GeneralAttributes.DamageEffect.REGULAR) {
            ...
            Main.getSoundPlayer().playSound(SFX.Enemies.GroundUnits.Paratroopers.ELECTRIFIED);
    ...
    public void onUpdate(float delta) {
        ...
        if (getElements().getEffectsManager().isBioHazardSmokeExists() && hasLanded()) {
            Main.getSoundPlayer().playSound(randomizer.nextBoolean() ? SFX.Enemies.GroundUnits.Paratroopers.COUGH_1 : SFX.Enemies.GroundUnits.Paratroopers.COUGH_2);
            onGib();
        }
        if (hasLanded()) {
            shoot();
    ...
    private void fallToDeath() {
        ...
        if (isYpositionEqualsGround()) {
            Main.getSoundPlayer().stopSound(myFallingSoundName, myFallingSoundId);
    ...
    private void playFallingSound() {
        ...
        myFallingSoundId = Main.getSoundPlayer().playSound(sound);
    }
...
//        createFlyingParts(Rules.GameObjectIds.BODY_PART, x, y, numberOfBodyParts);
        Main.getSoundPlayer().playSound(SFX.Misc.GIB);
    ...
    private final Timer.Task TASK_SHOOT = new Timer.Task() {
        ...
        public void run() {
            if (!dead && hasLanded() && target != null && !target.isDead()) {
                ...
                Main.getSoundPlayer().playSound(SFX.Enemies.GroundUnits.Paratroopers.INFANTRY_SHOOT);
            }
        }
    };
