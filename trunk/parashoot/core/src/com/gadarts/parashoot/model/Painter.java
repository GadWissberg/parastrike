...
public class Painter {
    ...
    public Painter(OrthographicCamera mainCamera, SpriteBatch batch, WarScreen warScreen) {
        ...
        this.batch = batch;
        messagesFont = Main.getAssetsManager().get(Rules.System.FontsParameters.ArmyFontNames.MEDIUM, BitmapFont.class);
        messagesFont.setColor(Color.RED);
        ShaderProgram.pedantic = false;
        flashRegion = ((TextureAtlas) Main.getAssetsManager().get(Assets.GFX.Sheets.InGame.Player.BONUS_APPENDIX_DATA_FILE)).findRegion(Assets.GFX.Sheets.ImagesNames.WHITE_FLASH);
        hudFont = Main.getAssetsManager().get(Rules.System.FontsParameters.DigitalFontNames.TINY, BitmapFont.class);
...
//            hudFont.draw(hudBatch, "IN SPATIAL POOL: " + warScreen.getElements().getInteractionsManager().getArraysPool().size, 10, Rules.System.Resolution.HEIGHT_TARGET_RESOLUTION - 400);
    ...
    public void setAlarm(boolean state) {
        alarmState = state;
        if (state) {
            Main.getSoundPlayer().playSound(SFX.Misc.ALARM, true, false);
        } else {
            Main.getSoundPlayer().stopSound(SFX.Misc.ALARM, -1);
        }
    }
