...
//    }
    ...
    private void resolveCannonsByAttribute(PlayerStats latestStats, PlayerStats olderStats, Rules.Player.UpgradeableAttribute.WeaponAttribute attribute) {
        for (BulletType cannon : Rules.Cannons.CANNONS_ORDER) {
            ...
            Main.getPlayerStatsHandler().setCannonAttribute(cannon, attribute, (latestCannonAttribute > olderCannonAttribute) ? latestCannonAttribute : olderCannonAttribute);
        }
    }
    ...
    private void resolveSideKicksAttributes(PlayerStats latestStats, PlayerStats olderStats, Rules.Player.UpgradeableAttribute attribute) {
        for (SideKickFactory.SideKickType sideKickType : SideKickFactory.SideKickType.values()) {
            ...
            Main.getPlayerStatsHandler().setSideKickAttribute(sideKickType, attribute, (latestAttribute > olderAttribute) ? latestAttribute : olderAttribute);
        }
    }

    private void resolveBunkerAttributes(PlayerStats latestStats, PlayerStats olderStats) {
        Main.getPlayerStatsHandler().setBunkerAttribute(Rules.Player.UpgradeableAttribute.PlayerCharacterAttribute.GENERATOR, latestStats.getBunkerGeneratorLevel() > olderStats.getBunkerGeneratorLevel() ? latestStats.getBunkerGeneratorLevel() : olderStats.getBunkerGeneratorLevel());
        Main.getPlayerStatsHandler().setBunkerAttribute(Rules.Player.UpgradeableAttribute.PlayerCharacterAttribute.ARMOR, latestStats.getBunkerArmorLevel() > olderStats.getBunkerArmorLevel() ? latestStats.getBunkerArmorLevel() : olderStats.getBunkerArmorLevel());
    }
