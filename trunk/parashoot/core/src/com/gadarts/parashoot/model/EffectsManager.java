...
public class EffectsManager {
    final TextureAtlas.AtlasRegion ALARM_REGION = ((TextureAtlas) Main.getAssetsManager().get(Assets.GFX.Sheets.InGame.HUD_DATA_FILE)).findRegion(Assets.GFX.Sheets.ImagesNames.ALARM);
    ...
    public void initializeEffects() {
        ...
        Main.getAssetsManager().getAll(ParticleEffect.class, effectsFiles);
        for (int i = 0; i < effectsFiles.size; i++) {
            ParticleEffect effect = effectsFiles.get(i);
            effect.allowCompletion();
            String assetFileName = Main.getAssetsManager().getAssetFileName(effect);
        ...
        TextureAtlas textureAtlas = Main.getAssetsManager().get(Assets.GFX.Sheets.InGame.Misc.PARTICLES_DATA_FILE, TextureAtlas.class);
    ...
    public void onUpdate() {
        ...
        if (!painter.getAlarmState() && bunker.getHealth() <= Rules.Hud.Alarm.DAMAGE_CRITICAL_LIMIT && bunker.getHealth() > 0) {
            ...
            Main.getMentorsManager().readyMentorIfDidntRun(UPGRADE_ARMOR);
    ...
    private void handleQuakes() {
        if (quakeInterval != 0) {
            if (quakeInterval >= Rules.Level.GlobalEffects.Quake.MINIMUM_INTERVAL) {
                float deltaX = (elements.getMainCamera().position.x < originalXCamera) ? 1 : -1;
                float deltaY = (elements.getMainCamera().position.y < originalYCamera) ? 1 : -1;
                elements.getMainCamera().translate(deltaX * quakeInterval, deltaY * quakeInterval);
                if (quakeInterval > 0) {
    ...
    private final Timer.Task TASK_PLAY_RANDOM_AMB_SOUND = new Timer.Task() {
        @Override
        public void run() {
            Main.getSoundPlayer().playSound(ambSounds.random());
            Timer.instance().scheduleTask(TASK_PLAY_RANDOM_AMB_SOUND, (float) (Rules.Level.MIN_AMB_INTERVAL + Math.random() * Rules.Level.MIN_AMB_INTERVAL));
        }
    };
    ...
    private void shakeScreen(float quakeInterval, float decay, boolean autoStop) {
        if (this.quakeInterval == 0) {
            OrthographicCamera mainCamera = elements.getMainCamera();
    ...
    private final Timer.Task TASK_LIGHTNING = new Timer.Task() {
        ...
        public void run() {
            lightningOn = true;
            Main.getSoundPlayer().playSound(Math.random() > 0.5 ? SFX.Misc.THUNDER_1 : SFX.Misc.THUNDER_2, false);
    ...
    public void activateBioHazardSmoke() {
        if (biohazard == null) {
            ...
            Main.getSoundPlayer().playSound(SFX.Misc.FLAME_HIT);
        }
    }
    ...
    public void activateAtomicFlash() {
        ...
        Main.getSoundPlayer().playSound(SFX.Player.Bonus.ATOM);
    }
